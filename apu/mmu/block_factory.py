...
async def auto_execute_block_proposal(proposal: dict) -> bool:
    """
    Takes a block proposal dict (generated by detect_new_block_opportunity)
    and automatically executes the local & external creation pipeline.
    """
    ...
    try:
        ...
        await create_dynamic_block(
            ...
            wcd_client=wcd_client
        )
        return True
    except Exception as e:
        logger.error("Failed to auto-execute block proposal: %s", e)
        raise e
